...
validated AS (
  SELECT
    ...
    TRIM(
      ...
      CASE WHEN serviceType IS NULL OR TRIM(serviceType) = '' THEN 'ERR_REQUIRED_serviceType; ' END
...
final_rows AS (
  ...
  SELECT
    ...
    NULL,
    NULL,
    'ERR_NO_SOURCE_ROWS_FOR_IEPSERVICE; '
  FROM dual
...
FROM (
    SELECT 0 AS sort_order,
           '"row_type","studentNum","startDate","serviceType","validation_errors"' AS csv_line
    FROM dual
