/* ============================================================================
  ...
  Target file: hearingScreening.csv
...
ps_source AS (
  ...
  WHERE a.dateexpires IS NOT NULL
    AND (
         UPPER(a.description) LIKE '%HEAR%'
      OR UPPER(a.description) LIKE '%AUDIO%'
...
final_rows AS (
  ...
  SELECT
    ...
    CAST(NULL AS DATE),
    CAST('ERR_NO_SOURCE_ROWS_FOR_HEARINGSCREENING; ' AS VARCHAR2(200))
  FROM dual
