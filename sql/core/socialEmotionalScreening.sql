/* ============================================================================
  ...
  Target file: socialEmotionalScreening.csv
...
ps_source AS (
    ...
    WHERE a.dateexpires IS NOT NULL
      AND (
            UPPER(a.description) LIKE '%SOCIAL%'
         OR UPPER(a.description) LIKE '%EMOT%'
         OR UPPER(a.description) LIKE '%BEHAV%'
         OR UPPER(a.description) LIKE '%SEL%'
      )
),
...
final_rows AS (
    ...
    SELECT
        ...
        CAST(NULL AS DATE),
        'ERR_NO_SOURCE_ROWS_FOR_SOCIALEMOTIONALSCREENING; '
    FROM dual
