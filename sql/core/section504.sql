...
ps_source AS (
  SELECT
    ss.studentNum,
    TRUNC(sx.whencreated) AS startDate
  FROM student_scope ss
  JOIN ps.s_nd_stu_x sx
    ON sx.studentsdcid = ss.dcid
  WHERE sx.section504 = 1
),
...
final_rows AS (
  ...
  SELECT
    ...
    NULL,
    NULL,
    'ERR_NO_SOURCE_ROWS_FOR_SECTION504; '
  FROM dual
