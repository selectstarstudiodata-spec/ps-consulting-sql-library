/* ============================================================================
  ...
  Target file: scoliosisScreening.csv
...
ps_source AS (
  ...
  WHERE a.dateexpires IS NOT NULL
    AND (
         UPPER(a.description) LIKE '%SCOLIO%'
      OR UPPER(a.description) LIKE '%SPINE%'
...
final_rows AS (
  ...
  SELECT
    ...
    CAST(NULL AS DATE),
    CAST('ERR_NO_SOURCE_ROWS_FOR_SCOLIOSISSCREENING; ' AS VARCHAR2(200))
  FROM dual
