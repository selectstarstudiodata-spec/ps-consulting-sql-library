/* ============================================================================
  ...
  Target file: dentalScreening.csv
...
ps_source AS (
  ...
  WHERE a.dateexpires IS NOT NULL
    AND (
         UPPER(a.description) LIKE '%DENT%'
      OR UPPER(a.description) LIKE '%ORAL%'
...
final_rows AS (
  ...
  SELECT
    ...
    CAST(NULL AS DATE),
    CAST('ERR_NO_SOURCE_ROWS_FOR_DENTALSCREENING; ' AS VARCHAR2(200))
  FROM dual
