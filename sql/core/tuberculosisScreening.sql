/* ============================================================================
  ...
  Target file: tuberculosisScreening.csv
...
ps_source AS (
    ...
    WHERE a.dateexpires IS NOT NULL
      AND (
            UPPER(a.description) LIKE '%TB%'
         OR UPPER(a.description) LIKE '%TUBERC%'
         OR UPPER(a.description) LIKE '%PPD%'
         OR UPPER(a.description) LIKE '%MANTOUX%'
      )
),
...
final_rows AS (
    ...
    SELECT
        ...
        CAST(NULL AS DATE),
        'ERR_NO_SOURCE_ROWS_FOR_TUBERCULOSISSCREENING; '
    FROM dual
