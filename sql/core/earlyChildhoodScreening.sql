/* ============================================================================
  ...
  Target file: earlyChildhoodScreening.csv
...
ps_source AS (
  ...
  WHERE a.dateexpires IS NOT NULL
    AND (
         UPPER(a.description) LIKE '%EARLY%'
      OR UPPER(a.description) LIKE '%PRESCHOOL%'
      OR UPPER(a.description) LIKE '%KINDERGARTEN SCREEN%'
      OR UPPER(a.description) LIKE '%ECSE%'
      OR UPPER(a.description) LIKE '%CHILD FIND%'
    )
),
...
final_rows AS (
  ...
  SELECT
    ...
    CAST(NULL AS DATE),
    CAST('ERR_NO_SOURCE_ROWS_FOR_EARLYCHILDHOODSCREENING; ' AS VARCHAR2(200))
  FROM dual
