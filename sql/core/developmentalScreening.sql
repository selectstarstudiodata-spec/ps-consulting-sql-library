/* ============================================================================
  ...
  Target file: developmentalScreening.csv
...
ps_source AS (
  ...
  WHERE a.dateexpires IS NOT NULL
    AND (
         UPPER(a.description) LIKE '%DEVELOP%'
      OR UPPER(a.description) LIKE '%DEV SCREEN%'
      OR UPPER(a.description) LIKE '%EARLY CHILD%'
      OR UPPER(a.description) LIKE '%PRESCHOOL SCREEN%'
    )
),
...
final_rows AS (
  ...
  SELECT
    ...
    CAST(NULL AS DATE),
    CAST('ERR_NO_SOURCE_ROWS_FOR_DEVELOPMENTALSCREENING; ' AS VARCHAR2(200))
  FROM dual
