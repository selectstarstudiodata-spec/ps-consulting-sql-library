/* ============================================================================
  ...
  Target file: heightWeightScreening.csv
  ...
  - Source mapping prefers real logged events over placeholders.
...
ps_source AS (
  ...
  WHERE a.dateexpires IS NOT NULL
    AND (
         UPPER(a.description) LIKE '%HEIGHT%'
      OR UPPER(a.description) LIKE '%WEIGHT%'
      OR UPPER(a.description) LIKE '%BMI%'
      OR UPPER(a.description) LIKE '%PHYSICAL%'
    )
),
...
final_rows AS (
  ...
  SELECT
    ...
    CAST(NULL AS DATE),
    CAST('ERR_NO_SOURCE_ROWS_FOR_HEIGHTWEIGHTSCREENING; ' AS VARCHAR2(200))
  FROM dual
