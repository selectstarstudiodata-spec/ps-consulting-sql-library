/* ============================================================================
  ...
  Target file: sportsScreening.csv
...
ps_source AS (
    ...
    WHERE a.dateexpires IS NOT NULL
      AND (
            UPPER(a.description) LIKE '%SPORT%'
         OR UPPER(a.description) LIKE '%ATHLET%'
         OR UPPER(a.description) LIKE '%PHYSICAL%'
         OR UPPER(a.description) LIKE '%CLEARANCE%'
      )
),
...
final_rows AS (
    ...
    SELECT
        ...
        CAST(NULL AS DATE),
        'ERR_NO_SOURCE_ROWS_FOR_SPORTSSCREENING; '
    FROM dual
