/* ============================================================================
  ...
  Target file: visionScreening.csv
...
ps_source AS (
  ...
  WHERE a.dateexpires IS NOT NULL
    AND (
         UPPER(a.description) LIKE '%VISION%'
      OR UPPER(a.description) LIKE '%EYE%'
...
final_rows AS (
  ...
  SELECT
    ...
    CAST(NULL AS DATE),
    CAST('ERR_NO_SOURCE_ROWS_FOR_VISIONSCREENING; ' AS VARCHAR2(200))
  FROM dual
